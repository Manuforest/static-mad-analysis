...
Always read [core-understanding.md](references/core-understanding.md), then select the smallest adapter set supported by observable media behavior:
...
### 4. Apply adapters and diagnose arrangement
...
### 6. Add craft or reception only when relevant
...
Use [report-template.md](assets/report-template.md) for a detailed report. Lead with the main interpretation and confidence, then show the observable timeline, subject map, event chains, time-space, audiovisual organization, adapter findings, and three-layer synthesis.
